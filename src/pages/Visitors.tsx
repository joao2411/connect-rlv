      ...
      <div className="max-w-4xl mx-auto">
              ...
              <form onSubmit={handleSave} className="space-y-4 mt-2">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input id="v-phone" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="(11) 99999-9999" />
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="v-date">Primeira visita *</Label>
                    <Input id="v-date" type="date" value={form.first_visit_date} onChange={(e) => setForm({ ...form, first_visit_date: e.target.value })} required />
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="v-invited">Convidado por</Label>
                  <Input id="v-invited" value={form.invited_by} onChange={(e) => setForm({ ...form, invited_by: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="v-how">Como conheceu a igreja</Label>
                  <Input id="v-how" value={form.how_found_us} onChange={(e) => setForm({ ...form, how_found_us: e.target.value })} />
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="v-obs">Observações</Label>
                  <Textarea id="v-obs" value={form.observations} onChange={(e) => setForm({ ...form, observations: e.target.value })} rows={3} />
                </div>
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-xl border border-border p-12 text-center text-muted-foreground">
            {search ? "Nenhum visitante encontrado." : "Nenhum visitante cadastrado. Clique em '+ Novo Visitante'."}
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map((v) => (
              <div key={v.id} className="bg-card rounded-xl border border-border p-5 flex items-start justify-between gap-4 hover:shadow-sm transition-shadow">
                  ...
                  <Dialog open={deleteId === v.id} onOpenChange={(o) => !o && setDeleteId(null)}>
                    ...
                    <DialogContent className="sm:max-w-sm">
                      ...
                      <div className="flex gap-3 mt-4">
                        ...
                        <Button variant="destructive" className="flex-1" onClick={() => handleDelete(v.id)}>Remover</Button>
                      </div>
                    </DialogContent>
                  </Dialog>
                </div>
